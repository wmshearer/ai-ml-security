...
VECTORDB_ROOT = Path(__file__).resolve().parent.parent
EVIDENCE_ROOT = VECTORDB_ROOT / "evidence"


def read_evidence(product: str, filename: str) -> str:
    ...
    path = EVIDENCE_ROOT / product / filename
...
def require_live(name: str, host: str, port: int):
    ...
    Tests that need a live instance call this at the top."""
    if not container_running(name):
        pytest.skip(f"container '{name}' is not running - start it with "
                     f"scripts/run_{name.split('-')[-1]}.sh to run this live test")
